...
//   short-circuit the request.
  ...
  for (var i = 0; i < src.length; i++) {
    ...
    if (expr.charAt(0) === '=') {
      ...
      for (var j = 0; j < spl.length - 1; j++) {
        var inverse = spl[j].trim()[0] === '!', attr_name = spl[j].trim().substr(inverse ? 1 : 0);
  ...
  return function (el) {
    var curClass = null, wasAttached = false, callback
      ;

    callback = function (_ev, newVal, _oldVal) {
      var nowAttached = $(el).closest('body').length > 0, newClass = null
        ;
...
*/
  ...
  var args = can.makeArray(arguments).slice(0, arguments.length - 1), options = arguments[arguments.length - 1], attribs = [], that = this.___st4ck ? this[this.length - 1] : this, data = can.extend({}, that), hash = quickHash(args.join('-'), quickHash(that._cid)).toString(36), attr_count = 0;
  ...
  var that = this, _val1 = resolve_computed(val1), _val2 = resolve_computed(val2);
  function exec() {
...
// Checks whether any hooks are registered for a particular key
  ...
  var hook, tag_name = tag_prefix.split(' ')[0]
    ;
  ...
  function hookup(element, parent, view_id) {
    var $element = $(element), f = function () {
        var g = deferred && deferred.state() === 'rejected' ? funcs.fail : funcs.done, args = arguments, term = element.nextSibling, compute = can.compute(function () { return g.apply(this, args) || ''; }, this)
        ;
  ...
  var model = CMS.Models[type] || GGRC.Models[type], $dummy_content = $(options.fn({}).trim()).first(), tag_name = $dummy_content.prop('tagName'), context = this.instance ? this.instance : this instanceof can.Model.Cacheable ? this : null, require_permission = '', items_dfd, hook;
  ...
  function hookup(element, parent, view_id) {
    items_dfd.done(function (items) {
      var val, $parent = $(element.parentNode), $el = $(element);
  ...
  var i = 0, j = 0, args = can.makeArray(arguments).slice(0, arguments.length - 1), options = arguments[arguments.length - 1], step = options.hash && options.hash.step || 1, ctx = {}, ret = [];
...
// Iterate over a string by spliting it by a separator
  ...
  var selected_option = object.attr(attr_name), selected_id = selected_option ? selected_option.id : null, options_dfd = CMS.Models.Option.for_role(role), tabindex = options.hash && options.hash.tabindex, tag_prefix = 'select class="span12"'
    ;

  function get_select_html(options) {
    return [
      '<select class="span12" model="Option" name="' + attr_name + '"', tabindex ? ' tabindex=' + tabindex : '', '>', '<option value=""', !selected_id ? ' selected=selected' : '', '>---</option>', can.map(options, function (option) {
          return [
            '<option value="', option.id, '"', selected_id === option.id ? ' selected=selected' : '', '>', option.title, '</option>'
          ].join('');
        }).join('\n'), '</select>'
    ].join('');
  }
  ...
  var selected_options = object[attr_name] || [] //object.attr(attr_name) || []
    ...
    }), options_dfd = CMS.Models[category_type].findAll(), tab_index = options.hash && options.hash.tabindex, tag_prefix = 'select class="span12" multiple="multiple"'
      ;

  tab_index = typeof tab_index !== 'undefined' ? ' tabindex="' + tab_index + '"' : '';
  function get_select_html(options) {
    return [
      '<select class="span12" multiple="multiple"', ' model="' + category_type + '"', ' name="' + attr_name + '"', tab_index, '>', can.map(options, function (option) {
          return [
            '<option value="', option.id, '"', selected_ids.indexOf(option.id) > -1 ? ' selected=selected' : '', '>', option.name, '</option>'
          ].join('');
        }).join('\n'), '</select>'
    ].join('');
  }
  ...
  return [
    '<a href="javascript://" class="show-long"', can.view.hook(function (el, parent, view_id) {
        el = $(el);
        ...
        if (content.length) {
          return ! function hide() {
            ...
            if (el[0].offsetHeight) {
              ...
              var root = el.closest('.tree-item'), toggle;
  ...
  var refresh_queue = new RefreshQueue(), context, frame = new can.Observe(), args = can.makeArray(arguments), i, arg;
  ...
  var context = arguments.length > 2 ? resolve_computed(options) : this, frame = new can.Observe(), loader, stack;
  ...
  var roles_deferred = new $.Deferred(), refresh_queue = new RefreshQueue()
    ;
  ...
  refresh_queue.trigger().then(function () {
    var user_roles = person.user_roles.reify(), user_roles_refresh_queue = new RefreshQueue()
      ;
  ...
  parent_instance = Mustache.resolve(parent_instance);
  var has_direct_mappings = false, has_external_mappings = false, mappings_type = '', i
    ;
  ...
  bindings = resolve_computed(bindings);
  parent_instance = Mustache.resolve(parent_instance);
  var has_extended_mappings = false, i
    ;
  ...
  var i, output = [], frame, length = list.length
    ;
  for (i = 0; i < length; i++) {
    frame = {
      index: i, isFirst: i === 0, isLast: i === length - 1, isSecondToLast: i === length - 2, length: length
    };
    frame[name] = list[i];
  ...
  var args = [].slice.apply(arguments), options = args.pop(), link = []
    ;
...
// To omit time pass in a second parameter {{date updated_at true}}
  ...
  if (typeof date === 'undefined')
    return '';
  var m = moment(new Date(date.isComputed ? date() : date)), dst = m.isDST(), no_time = arguments.length > 2
    ;
 ...
 */
  ...
  var args = Array.prototype.slice.call(arguments, 0), actions = [], resource, resource_type, context_unset = {}, context_id = context_unset, context_override, options = args[args.length - 1], passed = true
    ;
  ...
  can.each(instances, function (instance) {
    var resource_type, context_id, base_mappings = []
      ;
  ...
  */
  ...
  can.each({
  'localize_date': 'MM/DD/YYYY', 'localize_datetime': 'MM/DD/YYYY hh:mm:ss A'
}, function (tmpl, fn) {
  ...
  var args = can.makeArray(arguments), mappings = args.slice(1, args.length - 1), options = args[args.length - 1], root = options.contexts.attr('__mapping_count'), refresh_queue = new RefreshQueue(), mapping, dfd
    ;
  ...
  var dfd = $.when(), frame = new can.Observe(), user_roles = [], mappings, refresh_queue = new RefreshQueue()
    ;
...
// Determines and serializes the roles for a user
  ...
  instance = resolve_computed(instance);
  var state = options.contexts.attr('__infer_roles'), page_instance = GGRC.page_instance(), person = page_instance instanceof CMS.Models.Person ? page_instance : null, init_state = function () {
      if (!state.roles) {
          state.attr({
  ...
  if (!state.attr('status')) {
    if (person) {
      ...
      if (instance instanceof CMS.Models.Audit) {
        var requests = instance.requests || new can.Observe.List(), refresh_queue = new RefreshQueue()
          ;
...
// Uses search to find the counts for a model type
  ...
  if (!state.attr('status')) {
    ...
    if (!GGRC._search_cache_deferred) {
      ...
      var models = [
        'Program', 'Regulation', 'Contract', 'Policy', 'Standard', 'Section', 'Objective', 'Control', 'System', 'Process', 'DataAsset', 'Product', 'Project', 'Facility', 'OrgGroup', 'Audit', 'AccessGroup'
      ];
      GGRC._search_cache_deferred = GGRC.Models.Search.counts_for_types(null, models);
    }

    var model = CMS.Models[model_type], update_count = function (ev, instance) {
...
*/
  ...
  var args = arguments, options = arguments[arguments.length - 1], helper_result, helper_options = can.extend({}, options, {
      fn: function () { helper_result = 'fn'; }, inverse: function () { helper_result = 'inverse'; }
    })
    ;

  // Parse statements
  var statements = [], statement, match, disjunctions = [], index = 0
    ;
  can.each(args, function (arg, i) {
    if (i < args.length - 1) {
      if (typeof arg === 'string' && arg.match(/^\n\s*/)) {
        ...
        if (match = arg.match(/^\n\s*((and|or) )?([#^])?(\S+?)$/)) {
          statement = {
            fn_name: match[3] === '^' ? 'inverse' : 'fn', helper: Mustache.getHelper(match[4], options.contexts), args: [], logic: match[2] === 'or' ? 'or' : 'and'
          };

          // Add hash arguments
          if (options.hash) {
            var hash = {}, prefix = '_' + index + '_', prop
              ;
...
// {{#if_in_map roles 'role.permission_summary' 'Mapped'}}
  ...
  var auditors, auditors_dfd, decoy
    ;
...
*/
  ...
  var extra_attrs = '', args = can.makeArray(arguments).slice(1, arguments.length), options = args[args.length - 1] || helper, ret = '<script type=\'text/html\'' + can.view.hook(function (el, parent, view_id) {
  ...
  var audit, auditors_dfd, auditors, admin = Permission.is_allowed('__GGRC_ADMIN__'), editor = GGRC.current_user.system_wide_role === 'Editor', include_admin = !options.hash || options.hash.include_admin !== false;
  ...
  return function (el) {
    ...
    if (!prop || resolve_computed(prop)) {
      setTimeout(function () {
        $el.fadeIn({
          duration: (options.hash && options.hash.duration) || 500, complete: function () {
  ...
  var args = can.makeArray(arguments), options = args[args.length - 1]  // FIXME duplicate declaration
    , mapping_name
    ;
  ...
  var ctx = new can.Observe(), page_inst = GGRC.page_instance(), page_context = page_inst.context ? page_inst.context.id : null
    ;
  ...
  if (mappings) {
    ...
    mappings.attr && mappings.attr('length');
    mappings = can.map(mappings, function (item, key) {
      var mp = item.get_mappings()[0], context_id = mp.context ? mp.context.id : null
        ;
