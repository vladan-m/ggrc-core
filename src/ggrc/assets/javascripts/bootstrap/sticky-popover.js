...
========================================== */
  ...
  StickyPopover.prototype = $.extend({}, $.fn.popover.Constructor.prototype, {
      ...
      if (force || this.displayState !== 'show') {
        if (this.displayState !== 'show') {
          ...
          this.tip().css({opacity:
            window.getComputedStyle(this.tip()[0]).opacity
          });
        }
    ...
    trigger_load: function () {
      var self = this;
      var href = this.$element.data('popover-href');
      var loaded = this.$element.data('popover-loaded');
      ...
      if (!loaded) {
        $.ajax({
          ...
          success: function (data) {
            var $data = $(data.trim());
            self.$element.attr('data-original-title',
              $data.filter('.popover-title').html());
            self.$element.attr('data-content',
              $data.filter('.popover-content').html());
 ...
 * ======================= */
  $.fn.sticky_popover = function (option) {
    return this.each(function () {
      var $this = $(this);
      var data = $this.data('sticky_popover');
