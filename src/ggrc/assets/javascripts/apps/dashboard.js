...
(function (namespace, $, GGRC) {
  ...
  var admin_list_descriptors = {
    people: {
      ...
      init: function () {
        var self = this;
        CMS.Models.Role.findAll({scope__in: 'System,Admin'})
          .done(function (roles) {
    ...
    roles: {
      ...
      extra_params: {
        scope__in: 'System,Admin,Private Program,Workflow'
      },
    ...
    Person: {
      ...
      page_title: function (controller) {
        ...
        return /dashboard/.test(window.location) ?
              'GRC: My Work' :
              'GRC Profile: ' + (instance.name && instance.name.trim()) ||
  ...
  $('body').on('click', '.note-trigger, .edit-notes', function (ev) {
    ...
    var notes_model = GGRC.widget_descriptors[type].model;
