...
(function (can, $, Mousetrap) {
  ...
  CMS.Controllers.Filterable('CMS.Controllers.QuickSearch', {
    defaults: {
    list_view: GGRC.mustache_path + '/dashboard/object_list.mustache', spin: true, tab_selector: 'ul.nav-tabs:first > li > a', tab_href_attr: ['href', 'data-tab-href'], tab_target_attr: ['data-tab-target', 'href'], tab_model_attr: ['data-model', 'data-object-singular'], limit: null
  }
  }, {
  ...
  setup: function (el, opts) {
    ...
    $tabs.each(function (i, tab) {
      var $tab = $(tab), href = get_attr($tab, that.options.tab_href_attr), loaded = $tab.data('tab-loaded'), pane = get_attr($tab, that.options.tab_target_attr), $pane = $(pane), template = $tab.data('template') || that.options.list_view, model_name = get_attr($tab, that.options.tab_model_attr), model = can.getObject('CMS.Models.' + model_name) || can.getObject('GGRC.Models.' + model_name), view_data = null, xhrs = {};
      ...
      if (model && template) {
        view_data = new can.Observe({
          list: new model.List(), all_items: new model.List(), filtered_items: new model.List(), observer: that.options.observer, model: model
        });
  ...
  , redo_last_filter: function (id_to_add) {
      ...
      $tabs.each(function (i, tab) {
      var $tab = $(tab), model = $tab.data('model'), res = old_ids ? that.last_filter.getResultsFor(model) : null, view_data = $tab.data('view_data');
  ...
  can.Control('CMS.Controllers.LHN', {
    ...
    var $area = $('.area'), $lhsHolder = $('.lhs-holder'), $bar = $('.bar-v'), $lhnTrigger = $('.lhn-trigger')
        ;
    ...
    window.resize_areas();
      ...
      this._value = value;
    }, mousedown: false, dragged: false, resize_lhn: function (resize, no_trigger) {
  ...
  can.Control('CMS.Controllers.InfiniteScroll', {
    ...
    init: function () {
    }, ' DOMMouseScroll': 'prevent_overscroll', ' mousewheel': 'prevent_overscroll', ' scroll': 'prevent_overscroll', prevent_overscroll: function ($el, ev) {
      // Based on Troy Alford's response on StackOverflow:
      //   http://stackoverflow.com/a/16324762
      var scrollTop = $el[0].scrollTop, scrollHeight = $el[0].scrollHeight, height = $el.height(), scrollTopMax = scrollHeight - height, delta, up, loadTriggerOffset = 50
        ;
  ...
  can.Control('CMS.Controllers.LHN_Search', {
    defaults: {
      list_view: GGRC.mustache_path + '/base_objects/search_result.mustache', actions_view: GGRC.mustache_path + '/base_objects/search_actions.mustache', list_selector: 'ul.top-level > li, ul.mid-level > li', list_toggle_selector: 'li > a.list-toggle', model_attr_selector: null, model_attr: 'data-model-name', model_extra_attr: 'data-model-extra', count_selector: '.item-count', list_mid_level_selector: 'ul.mid-level', list_content_selector: 'ul.sub-level', actions_content_selector: 'ul.sub-actions', limit: 50, observer: null, filter_params: new can.Observe(), counts: new can.Observe()
    }
  }, {
    display: function () {
      var self = this, prefs = this.options.display_prefs, prefs_dfd, template_path = GGRC.mustache_path + this.element.data('template')
        ;
      ...
      return can.view(template_path, prefs_dfd.then(function (prefs) { return prefs.getLHNState(); })).then(function (frag, xhr) {
        var lhn_prefs = prefs.getLHNState(), initial_term, initial_params = {}, saved_filters = prefs.getLHNState().filter_params
          ;
      ...
      can.Model.Cacheable.bind('created', function (ev, instance) {
        ...
        var visible_model_names =
              can.map(self.get_visible_lists(), self.proxy('get_list_model')), model_name = instance.constructor.shortName
              ;
      ...
      var sub_selector = this.options.list_content_selector + ',' + this.options.actions_content_selector, mid_selector = this.options.list_mid_level_selector, $parent = el.parent('li'), selector
        ;
      ...
      var holder = el.closest('.lhs-holder'), $content = $ul.filter([this.options.list_content_selector,
                               this.options.list_mid_level_selector].join(',')), $siblings = selector ? $ul.closest('.lhs').find(selector) : $(false), extra_height = 0, top
                               ;
      ...
      if ($content.length) {
        var last_height = this._holder_height, holder = this.element.closest('.lhs-holder')
          ;
        this._holder_height = holder.outerHeight();
      ...
      var $list = $(el).closest(this.get_lists()), model_name = this.get_list_model($list), tracker_stop = GGRC.Tracker.start('LHN_show_list', model_name), that = this
        ;
    ...
    }, '.sub-level scrollNext': 'show_more', show_more: function ($el, ev) {
    if (this._show_more_pending)
        return;

    var that = this, $list = $el.closest(this.get_lists()), model_name = this.get_list_model($list), visible_list = this.options.visible_lists[model_name], results_list = this.options.results_lists[model_name], refresh_queue, new_visible_list, tracker_stop = GGRC.Tracker.start('LHN', 'show_more', model_name)
        ;
    ...
    visible_list.attr('is_loading', true);
      ...
      can.each(this.get_lists(), function ($list) {
        ...
        var context = {
          model: CMS.Models[model_name], filter_params: self.options.filter_params, list: self.options.visible_lists[model_name], counts: self.options.counts, count: can.compute(function () {
      ...
      var self = this, lists = this.get_visible_lists(), dfds = [], search_text = this.current_term, my_work = self.current_params && self.current_params.contact_id, extra_params = self.current_params && self.current_params.extra_params
        ;

      can.each(lists, function (list) {
        var dfd, $list = $(list), model_name = self.get_list_model($list), results = search_result.getResultsForType(model_name), refresh_queue = new RefreshQueue(), initial_visible_list = null;
        ...
        dfd = refresh_queue.trigger().then(function (d) {
          new CMS.Models.LocalListCache({
            name: 'search_' + model_name, objects: d, search_text: search_text, my_work: my_work, extra_params: extra_params, type: model_name, keys: ['title', 'contact', 'private', 'viewLink']
          }).save();
          return d;
        });
        if (display_now) {
      ...
      var self = this, search_id = this.search_id, models, extra_models;
      ...
      var self = this, search_id = this.search_id, lists = this.get_visible_lists(), models = can.map(lists, this.proxy('get_list_model'))
        ;
      ...
      var self = this, tracker_stop = GGRC.Tracker.start(
          'LHN_run_search',
          extra_params && extra_params.contact_id ? 'MyWork' : 'Normal'), filter_list = [];
  ...
  can.Control('CMS.Controllers.LHN_Tooltips', {
    defaults: {
      tooltip_view: GGRC.mustache_path + '/base_objects/extended_info.mustache', trigger_selector: '.show-extended', fade_in_delay: 300, fade_out_delay: 300
    }
  }, {
    init: function () {
    ...
    , '{trigger_selector} mouseenter': 'on_mouseenter', '{trigger_selector} mouseleave': 'on_mouseleave', '{$extended} mouseleave': 'on_mouseleave', '{$extended} mouseenter': 'on_tooltip_mouseenter', on_mouseenter: function (el, ev) {
        var instance = el.closest('[data-model]').data('model') ||
                      el.closest(':data(model)').data('model'), delay = this.options.fade_in_delay
                      ;
      ...
      var offset = this.options.$extended.offset().top, height = this.options.$extended.height()
        ...
        , window_height = $(window).height() + $(window).scrollTop() -
            (this.options.$extended.offset().left > 348 ? 0 : 24), new_offset
            ;
      ...
      var self = this, tooltip_view = this.get_tooltip_view(el)
        ;
      ...
      if (tooltip_view) {
        ...
        can.view(tooltip_view, {instance: instance}, function (frag) {
          ...
          var tooltip_width = self.options.$extended.outerWidth(), offset = el.parent().offset(), el_left = offset ? offset.left : 0, offset_left = el_left - tooltip_width > 0 ?
