...
(function (can, $) {
  ...
  function createFolder(
    cls, title_generator, parent_attr, model, ev, instance) {
    var that = this;
    var dfd;
    var folder;
    var hasParent = true;
    ...
    if (instance instanceof cls) {
      if (parent_attr) {
        dfd = instance[parent_attr].reify().get_binding('folders')
          .refresh_instances();
      } else {
        hasParent = false;
      ...
      return dfd.then(function (parent_folders) {
        ...
        if (hasParent && parent_folders.length < 1) {
          return new $.Deferred().reject('no write permissions on parent');
        }
        xhr = new CMS.Models.GDriveFolder({
          title: title_generator(instance),
          parents: parent_folders.length ? parent_folders[0].instance :
            undefined
        }).save();
        report_progress('Creating Drive folder for ' +
          title_generator(instance), xhr);
        return xhr;
      }).then(function (created_folder) {
        folder = created_folder;

        return report_progress(
          'Linking folder "' + folder.title + '" to ' +
            instance.constructor.title_singular,
  ...
  function report_progress(str, xhr) {
    function build_flashes() {
      var flash = {};
      var container_string = '<div class=\'audit-status\'>';
      var head_string = '<div class=\'audit-status-head\'>';
      var list_bridge = ' [Click to open]</div><ul class=\'flash-expandable\'>';
      var closer = '</ul></div></div>';
      var successes = [container_string + head_string +
        'Actions completed successfully.' + list_bridge];
      var failures = [container_string + head_string + 'There were errors.' +
        list_bridge];
      var pendings = [container_string + head_string +
        'GDrive actions in progress...' + list_bridge];
      var success_count = 0;
      var failure_count = 0;
      var pending_count = 0;
  ...
  can.Control('GGRC.Controllers.GDriveWorkflow', {
    attach_files: function (files, type, object) {
      return new RefreshQueue().enqueue(files).trigger().then(function (files) {
        can.each(files, function (file) {
          ...
          CMS.Models.Document.findAll({link: file.alternateLink})
            .done(function (d) {
              if (d.length) {
    ...
    user_permission_ids: {},
    create_program_folder: partial_proxy(createFolder, CMS.Models.Program,
      function (inst) {
        return inst.title + ' Audits';
      }, null),
    /*
      NB: We don't do folder creation on Program creation now.  Only when the first Audit is created does the Program folder
      get created as well.
    */
    create_audit_folder: partial_proxy(createFolder, CMS.Models.Audit,
      function (inst) {
        return inst.title;
      }, 'program'),
    ...
    create_request_folder: partial_proxy(createFolder, CMS.Models.Request,
      function (inst) {
        return inst.objective.reify().title;
      }, 'audit'),
    link_request_to_new_folder_or_audit_folder: function (model, ev, instance) {
      if (!(instance instanceof CMS.Models.Request)) {
        return undefined;
      }
      if (this._audit_create_in_progress ||
        instance.audit.reify().object_folders.length < 1) {
        ...
        if (Permission.is_allowed('create', 'ObjectFolder',
          instance.context.id || null)) {
          return instance.delay_resolving_save_until(
            report_progress(
              'Linking new Request to Audit folder',
              new CMS.Models.ObjectFolder({
                folder_id: instance.audit.reify().object_folders[0]
                  .reify().folder_id,
                folderable: instance,
                context: instance.context || {id: null}
              }).save()
    ...
    resolve_permissions: function (folder, instance) {
      ...
      var push_person = function (dfds, role, email, queue_to_top) {
        if (!permissions[email] || permissions[email] !== 'writer' ||
          role !== 'reader') {
          email = email.toLowerCase();
          permissions[email] = role;
          if (!that.user_permission_ids[email]) {
            if (queue_to_top) {
              dfds.push(CMS.Models.GDriveFilePermission
                  .findUserPermissionId(email).then(function (permissionId) {
                    that.user_permission_ids[permissionId] = email;
                    that.user_permission_ids[email] = permissionId;
                  }));
            } else {
              return CMS.Models.GDriveFilePermission
                .findUserPermissionId(email).then(function (permissionId) {
                  that.user_permission_ids[permissionId] = email;
                  that.user_permission_ids[email] = permissionId;
                });
      ...
      can.each(permissionsByType[instance.constructor.model_singular],
        function (entry) {
          var role = entry.role;
          var people = typeof entry.value === 'string' ? instance[entry.value] :
            entry.value;
          var pdfd = [];
          ...
          function iterate_people(dfds, people) {
            can.each(!people || people.push ? people : [people],
              function (person) {
                if (person.person) {
      ...
      if (GGRC.config.GAPI_ADMIN_GROUP) {
        push_person(permission_dfds, 'writer',
          GGRC.config.GAPI_ADMIN_GROUP, true);
      }
      if (instance instanceof CMS.Models.Program) {
        ...
        instance.get_binding('authorizations').refresh_instances()
          .then(function (authmapping) {
            var auth;
            ...
            auth = authmapping.instance;
            if (!auths[auth.person.reify().email] ||
              auth.created_at.getTime() >
              auths[auth.person.reify().email].created_at.getTime()
            ) {
        ...
        can.each(auths, function (auth) {
          ...
          // only push valid emails
          if (rolesmap[role.name] && person.email) {
    ...
    update_permissions: function (model, ev, instance) {
      ...
      if (!(instance instanceof model) ||
        instance.__permissions_update_in_progress) {
        return $.when();
      }
      ...
      return instance.get_binding('folders').refresh_instances()
        .then(function (binding_list) {
          var binding_dfds = [];
          can.each(binding_list, function (binding) {
            binding_dfds.push($.when(
              ...
              permissions_to_delete = can.map(permissions_to_delete,
                function (permission) {
                  /*
                  ...
                  */
                  ...
                  switch (true) {
                    ...
                    case (pending_permission === permission.role):
                      delete permissions_to_create[email];
                      return undefined;
                    case (pending_permission !== 'reader' &&
              ...
              can.each(permissions_to_create, function (role, user) {
                permissions_dfds.push(
                  that.update_permission_for(binding.instance, user,
                    that.user_permission_ids[user], role).then(null,
                    function () {
                      return new $.Deferred().resolve(); // wait on these even if they fail.
                    }));
              });
    ...
    update_permission_for: function (item, person, permissionId, role) {
      ...
      if (item.userPermission.role !== 'writer' &&
        item.userPermission.role !== 'owner') {
        return $.Deferred().reject(
          'User is not authorized to modify this object');
      }
      if (person.email) {
        person = person.email;
      }
      ...
      return report_progress(
        'Creating ' + role + ' permission on folder "' +
          item.title + '" for ' + person,
        new CMS.Models.GDriveFolderPermission({
          folder: item,
          email: person,
          role: role,
          permission_type: person === GGRC.config.GAPI_ADMIN_GROUP ? 'group' :
            'user'
        }).save()
      );
    },
      ...
      can.each(audit_files, function (file) {
        ...
        tldfds.push(
          CMS.Models.ObjectDocument.findAll({
            ...
            var connected_request_responses = [];
            var other_responses = [];

            if (!connected_request_responses.length) {
              return $.Deferred.reject();
            }
            file.addToParent(new_folder);
            return new RefreshQueue().enqueue(other_responses).trigger()
              .then(function (reified_responses) {
                var dfds = [];
                reified_responses = can.map(reified_responses,
                  function (resp) {
                    if (!resp.request.reify().objective) {
                      return resp;
                    }
                  });
                if (reified_responses.length < 1) {
    ...
    move_files_to_audit_folder: function (instance, audit_folders, req_folders, files) {
      ...
      var move_dfds = [];
      can.each(files, function (file) {
        var parent_ids = can.map(file.parents, function (p) {
          return p.id;
        });
      ...
      report_progress('Moving files to the Audit folder',
        $.when.apply($, move_dfds).done(function () {
          can.each(req_folders, function (rf) {
            if (!rf.selfLink || rf.userPermission.role !== 'writer' &&
              rf.userPermission.role !== 'owner') {
              return;  // short circuit any operation if the user isn't allowed to add permissions
            }
            report_progress(
              'Checking whether folder "' + rf.title + '" is empty',
              CMS.Models.GDriveFile.findAll({parents: rf.id})
                .then(function (orphs) {
                  if (orphs.length < 1) {
                    report_progress(
                    ...
                    console.warn('can\'t delete folder',
                      rf.title, 'as files still exist:', orphs);
                  }
                }));
          });
        }));
    ...
    update_request_folder: function (model, ev, instance) {
      var that = this;
      if (!(instance instanceof CMS.Models.Request) ||
        instance._folders_mutex) {
        return undefined;
      }

      instance._folders_mutex = true;
      return instance.delay_resolving_save_until(
        $.when(instance.get_binding('folders').refresh_instances(),
          instance.audit.reify().get_binding('folders').refresh_instances()
        ).then(function (req_folders_mapping, audit_folders_mapping) {
          var audit_folder;
          var af_index;
          var obj_folder_to_destroy;
          var obj_destroy_dfds;
          var req_folders;
          var audit_folders;

          if (audit_folders_mapping.length < 1) {
            return undefined; // can't do anything if the audit has no folder
          }
          // reduce the binding lists to instances only -- makes for easier comparison
          req_folders = can.map(req_folders_mapping, function (rf) {
            return rf.instance;
          });
          // Check whether or not the current user has permissions to modify the audit folder.
          audit_folders = can.map(audit_folders_mapping, function (af) {
            if (!af.instance.selfLink ||
                af.instance.userPermission.role !== 'writer' &&
                af.instance.userPermission.role !== 'owner') {
              return undefined;  // short circuit any operation if the user isn't allowed to edit
            }
            return af.instance;
          });
          if (audit_folders.length < 1) {
            return undefined;  // no audit folder to work on, or none that the user can edit.
          }
          ...
          if (af_index > -1) {
            ...
            obj_folder_to_destroy = req_folders_mapping[
              can.inArray(audit_folder, req_folders)].mappings[0].instance;

            return $.when(
              report_progress(
                'Linking Request "' + instance.objective.reify().title +
                  '" to new folder',
              ...
              CMS.Models.GDriveFile.findAll({
                parentfolderid: audit_folders[0].id
              }),
              instance.responses.reify(),
              obj_folder_to_destroy.refresh().then(function (of) {
                return of.destroy();
              })
            ).then(
            ...
            obj_destroy_dfds = can.map(req_folders_mapping, function (b) {
              b.mappings[0].instance.refresh().then(function (of) {
                of.destroy();
              });
            ...
            return $.when(
              CMS.Models.GDriveFile.findAll({parents: req_folders[0].id}),
                $.when.apply($, obj_destroy_dfds).then(function () {
                  return can.unique(can.reduce(arguments, function (a, b) {
                    return a.concat(b);
                  }, []));
                })
            ).then(that.proxy('move_files_to_audit_folder',
              instance, audit_folders, req_folders)
            ).done(function () {
              return that.update_permissions(model, ev, instance);
            });
          }
    ...
    'a.create-folder click': function (el, ev) {
      var data = el.closest('[data-model], :data(model)').data('model') ||
        GGRC.make_model_instance(GGRC.page_object);
    ...
    create_folder_if_nonexistent: function (object) {
      ...
      if (parent_prop[object.constructor.shortName] &&
          !object.get_mapping('folders').length) {
        dfd = $.when(dfd, this.create_folder_if_nonexistent(
          object[parent_prop[object.constructor.shortName]].reify()));
      }
      return dfd.then(function foldercheck() {
        ...
        if (object.get_mapping('folders').length) {
          ...
          if (that._audit_create_in_progress ||
            object.audit.reify().get_mapping('folders').length < 1) {
            that.request_create_queue.push(object);
          } else {
            that.link_request_to_new_folder_or_audit_folder(
              object.constructor, {}, object);
          }
        } else {
          folder_dfd = that['create_' +
            object.constructor.table_singular + '_folder'](
              object.constructor, {}, object);

          if (object instanceof CMS.Models.Audit) {
            folder_dfd.done(function (fld) {
              new RefreshQueue().enqueue(object.requests.reify()).trigger()
                .done(function (reqs) {
    ...
    'a.show-meeting click': function (el, ev) {
      ...
      var cev = el.closest('[data-model], :data(cev)').data('cev');
      var subwin;
    ...
    scope: {
      ...
      _unlinkObjFolders: function (instance) {
        ...
        deleteDeferred = instance.refresh().then(function () {
          var deferredDeletes;
          var obj_folders = instance.object_folders;
    ...
    events: {
      init: function () {
        ...
        this.scope.instance.get_binding('folders').refresh_instances()
          .then(that._ifNotRemoved(function (folders) {
            that.scope.attr('current_folder', folders[0] ?
              folders[0].instance : null);
            ...
            if (!that.scope.instance.get_binding('extended_folders') ||
              that.scope.current_folder || that.scope.folder_error) {
              that.scope.removeAttr('_folder_change_pending');
              return;
            }
            that.scope.instance.get_binding('extended_folders')
              .refresh_instances().then(that._ifNotRemoved(function (folders) {
                that.scope.removeAttr('_folder_change_pending');
                that.scope.attr('current_folder', folders[0] ?
                  folders[0].instance : null);
                that.options.folder_list = folders;
      ...
      '{folder_list} change': function () {
        ...
        if (this.scope.deferred && this.scope.instance._pending_joins) {
          ...
          can.each(this.scope.instance._pending_joins.slice(0).reverse(),
            function (pj, i) {
      ...
      'a[data-toggle=gdrive-remover] click': function (el, ev) {
        ...
        if (!scope.deferred) {
          ...
          if (scope.current_folder) {
            ...
            can.each(scope.instance.object_folders.reify(),
        ...
        dfd.then(function () {
          if (scope.instance.get_binding('extended_folders')) {
            $.when(
              ...
              remote_folders = can.map(extended_bindings,
                function (folder_binding) {
                  return ~can.inArray(folder_binding.instance, self_folders) ?
                    undefined : folder_binding.instance;
                });
      ...
      'a[data-toggle=gdrive-picker] click': function (el, ev) {
        var dfd = GGRC.Controllers.GAPI.authorize(
  ...
  can.Component.extend({
    ...
    scope: {
      ...
      trigger_upload_parent: function (scope, el, ev) {
        ...
        function is_own_folder(mapping, instance) {
          ...
          if (!mapping.mappings || mapping.mappings.length < 1 ||
        ...
        verify_dfd.done(function () {
          if (that.instance.attr('_transient.folder')) {
            parent_folder_dfd = $.when([{
              instance: that.instance.attr('_transient.folder')
            }]);
          } else {
            parent_folder_dfd = that.instance.get_binding('extended_folders')
              .refresh_instances();
          }
          ...
          parent_folder_dfd.done(function (bindings) {
            ...
            parent_folder.uploadFiles().then(function (files) {
              ...
              return new RefreshQueue().enqueue(files).trigger()
                .then(function (fs) {
                  return $.when.apply($, can.map(fs, function (f) {
                    if (!~can.inArray(parent_folder.id, can.map(f.parents,
                      function (p) {
                        return p.id;
                      }))
                    ) {
              ...
              var files = can.map(can.makeArray(arguments),
                    function (file) {
      ...
      handle_file_upload: function (files) {
        ...
        can.each(files, function (file) {
          // Since we can re-use existing file references from the picker, check for that case.
          var dfd = CMS.Models.Document.findAll({link: file.alternateLink});
          dfd.then(function (d) {
            var doc_dfd;
            var object_doc;
    ...
    events: {
      ...
      '{scope} modal:success': function (scope, event) {
