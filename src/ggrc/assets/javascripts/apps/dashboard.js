...
(function (namespace, $) {
  ...
  var admin_list_descriptors = {
    'people': {
      model: CMS.Models.Person, roles: new can.Observe.List(), init: function () {
        var self = this;
        CMS.Models.Role.findAll({scope__in: 'System,Admin'}).done(function (roles) {
            self.roles.replace(sort_by_name_email(roles));
          });
      }, object_display: 'People', tooltip_view: '/static/mustache/people/object_tooltip.mustache', header_view: '/static/mustache/people/filters.mustache'  // includes only the filter, not the column headers
      , list_view: '/static/mustache/people/object_list.mustache', fetch_post_process: sort_by_name_email
    }, 'roles': {
      model: CMS.Models.Role, extra_params: {scope__in: 'System,Admin,Private Program,Workflow'}, object_category: 'governance', object_display: 'Roles', list_view: '/static/mustache/roles/object_list.mustache', fetch_post_process: sort_by_name_email
    }, 'events': {
      model: CMS.Models.Event, object_category: 'governance', object_display: 'Events', list_view: '/static/mustache/events/object_list.mustache'
    }, 'custom_attributes': {
  ...
  var admin_widgets = new GGRC.WidgetList('ggrc_admin', {
    admin: {
      'people': {
          'model': CMS.Models.Person, 'content_controller': GGRC.Controllers.ListView, 'content_controller_options': admin_list_descriptors['people'], 'widget_id': 'people_list', 'widget_icon': 'person', 'show_filter': false, widget_name: function () {
            return 'People';
          }, widget_info: function () {
            return '';
          }
        }, 'roles': {
          'model': CMS.Models.Role, 'content_controller': GGRC.Controllers.ListView, 'content_controller_options': admin_list_descriptors['roles'], 'widget_id': 'roles_list', 'widget_icon': 'role', 'show_filter': false, widget_name: function () {
            return 'Roles';
          }, widget_info: function () {
            return '';
          }
        }, 'events': {
          'model': CMS.Models.Event, 'content_controller': GGRC.Controllers.ListView, 'content_controller_options': admin_list_descriptors['events'], 'widget_id': 'events_list', 'widget_icon': 'event', widget_name: function () {
            return 'Events';
          }, widget_info: function () {
            return '';
          }
  ...
  extra_page_options = {
    Program: {
      header_view: GGRC.mustache_path + '/base_objects/page_header.mustache', page_title: function (controller) {
  ...
  if (/^\/\w+\/\d+($|\?|\#)/.test(location) || /^\/dashboard/.test(location) ||
    ...
    $area.cms_controllers_page_object($.extend({
      ...
      widget_descriptors: GGRC.WidgetList.get_widget_list_for(model_name), default_widgets: defaults || GGRC.default_widgets || [], instance: GGRC.page_instance(), header_view: GGRC.mustache_path + '/base_objects/page_header.mustache', page_title: function (controller) {
    ...
    $area.cms_controllers_dashboard({
      widget_descriptors: GGRC.WidgetList.get_widget_list_for('admin'), menu_tree_spec: GGRC.admin_menu_spec, default_widgets: ['people', 'roles', 'events', 'custom_attributes']
    });
    init_widgets();
  ...
  $('body').on('click', '.note-trigger, .edit-notes', function (ev) {
    ev.stopPropagation();
    var $object = $(ev.target).closest('[data-object-id]'), type = $object.data('object-type'), notes_model = widget_descriptors[type].model, sec;

    $(ev.target).closest('.note').cms_controllers_section_notes({
      section_id: $object.data('object-id') || (/\d+$/.exec(window.location.pathname) || [null])[0], model_class: notes_model
    });
  });
})(this, jQuery);
