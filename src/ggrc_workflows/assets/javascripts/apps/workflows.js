...
(function ($, CMS, GGRC) {
  var draftOnUpdateMix;
  var WorkflowExtension = {};

  var _workflow_object_types = [
    'Program', 'Regulation', 'Policy', 'Standard', 'Contract', 'Clause',
    'Section', 'Request', 'Control', 'Objective', 'OrgGroup', 'Vendor',
    'AccessGroup', 'System', 'Process', 'DataAsset', 'Product', 'Project',
    'Facility', 'Market', 'Issue', 'ControlAssessment', 'Risk', 'Threat'
  ];

  var _task_sort_function = function (first, second) {
    var dateA = Number(new Date(first.end_date));
    var dateB = Number(new Date(second.end_date));
  ...
  WorkflowExtension.init_mappings = function () {
    ...
    var mappings = {
      TaskGroup: {
        ...
        objects: Proxy(
          null, 'object', 'TaskGroupObject',
          'task_group', 'task_group_objects'),
        workflow: Direct(
      ...
      Workflow: {
        ...
        previous_cycles:
          CustomFilter('cycles', function (result) {
        ...
        current_task_group_objects: Cross(
            'current_task_groups', 'cycle_task_group_objects_for_page_object'),
        current_tasks: Cross(
          'current_task_groups',
          'cycle_task_group_object_tasks_for_page_object'),
        current_all_tasks:
          Cross('current_task_groups', 'cycle_task_group_tasks'),
        ...
        mapped_and_or_authorized_people:
          Multi(['people', 'authorized_people']),
        roles: Cross('authorizations', 'role'),

        // This is a dummy mapping that ensures the WorkflowOwner role is
        // loaded before we do the custom filter for owner_authorizations.
        authorizations_and_roles: Multi(['authorizations', 'roles']),

        owner_authorizations:
          CustomFilter('authorizations_and_roles', function (binding) {
            return (
              binding.instance instanceof CMS.Models.UserRole &&
        ...
        orphaned_objects: Multi([
          'cycles', 'task_groups', 'tasks',
          'current_task_groups', 'current_tasks', 'folders'
        ])
      },
      ...
      CycleTaskGroup: {
        ...
        cycle_task_group_object_tasks_for_page_object: Cross(
          'cycle_task_group_objects_for_page_object',
      ...
      CycleTaskEntry: {
        ...
        cycle: Direct('Cycle', 'cycle_task_entries', 'cycle'),
    ...
    can.each(_workflow_object_types, function (type) {
      ...
      mappings[type] = {
        task_groups:
          new GGRC.ListLoaders.ProxyListLoader(
            'TaskGroupObject', 'object', 'task_group',
            'task_group_objects', null),

        cycle_objects:
          Direct('CycleTaskGroupObject', 'object', 'cycle_task_group_objects'),

        object_tasks:
          Cross('cycle_objects', 'cycle_task_group_object_tasks'),
  ...
  WorkflowExtension.init_widgets = function () {
    ...
    _.each(_workflow_object_types, function (type) {
      ...
      treeWidgets[type] = treeWidgets[type].concat([
        'TaskGroup', 'Workflow', 'CycleTaskEntry',
  ...
  WorkflowExtension.init_widgets_for_other_pages = function () {
    ...
    if (
      pageInstance &&
      ~can.inArray(pageInstance.constructor.shortName, _workflow_object_types)
    ) {
          ...
          content_controller_options: {
            ...
            child_options: [{
              ...
              footer_view:
                (GGRC.mustache_path +
                '/cycle_task_group_object_tasks' +
                '/current_tg_tree_footer.mustache')
            }]
          }
        },
        task: {
    ...
    new GGRC.WidgetList(
      'ggrc_workflows', descriptor, ['info_widget', 'task_widget']);
  };
  ...
  WorkflowExtension.init_widgets_for_workflow_page = function () {
    ...
    can.each(
      ...
      function (descriptor, name) {
        if (newDefaultWidgets.indexOf(name) >= 0) {
    ...
    new GGRC.WidgetList(
      'ggrc_workflows', {Workflow: newWidgetDescriptors});

    // Setup extra refresh required due to automatic creation of
    // permissions on creation of WorkflowPerson
    CMS.Models.WorkflowPerson.bind('created', function (ev, inst) {
      if (inst instanceof CMS.Models.WorkflowPerson && inst.context) {
        inst.context.reify().refresh();
      }
    });
  };
  ...
  GGRC.register_hook(
      'LHN.Sections_workflow',
      GGRC.mustache_path + '/dashboard/lhn_workflows');
  ...
  draftOnUpdateMix = can.Model.Mixin(
    {}, {
