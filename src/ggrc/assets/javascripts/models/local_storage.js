  ...
  can.Model('can.Model.LocalStorage', {
    makeFindOne: function (findOne) {
      ...
      if (typeof findAll === 'function' && this !== can.Model.LocalStorage) {
        ...
        return function (params, success, error) {
          var def = new can.Deferred(), key = [this._shortName, 'ids'].join(':'), data = window.localStorage.getItem(key), returns = new can.Model.List(), that = this;
          params = params || {};

          if (data) {
            can.each(JSON.parse(data), function (id) {
              if (params.id === null || params.id === id) {
                var k = [that._shortName, id].join(':'), d = window.localStorage.getItem(k);
      ...
      if (typeof create === 'function' && this !== can.Model.LocalStorage) {
        ...
        return function (params) {
          var key = [this._shortName, 'ids'].join(':'), data = window.localStorage.getItem(key), newkey = 1, def = new can.Deferred()
            ;
      ...
      if (typeof update === 'function' && this !== can.Model.LocalStorage) {
        ...
        return function (id, params) {
          var key = [this._shortName, id].join(':'), data = window.localStorage.getItem(key), def = new can.Deferred()
            ;
      ...
      if (typeof findAll === 'function' && this !== can.Model.LocalStorage) {
        ...
        return function (id) {
          var def = new can.Deferred(), key = [this._shortName, id].join(':'), item = this.model({id: id});
