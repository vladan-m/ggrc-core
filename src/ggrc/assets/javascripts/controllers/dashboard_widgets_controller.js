...
CMS.Controllers.Filterable('CMS.Controllers.DashboardWidgets', {
  defaults: {
    model: null, widget_id: '', widget_name: '', widget_icon: '', widget_view: '/static/mustache/dashboard/object_widget.mustache', widget_guard: null, widget_initial_content: '', show_filter: false, object_category: null //e.g. "governance"
    , content_selector: '.content'
    //, minimum_widget_height : 100
    , content_controller: null, content_controller_options: {}, content_controller_selector: null
  }
}, {
  ...
  init: function () {
    ...
    this._prepare_deferred = $.when(
        can.view(this.options.widget_view, $.when(this.options)), CMS.Models.DisplayPrefs.findAll()
    ...
    return this._prepare_deferred;
      ...
      var content = this.element, controller_content = null;
      if (prefs.length < 1) {
      ...
      if (this.options.content_controller) {
        ...
        this.content_controller = new this.options.content_controller(
            controller_content, this.options.content_controller_options
        );
