...
(function (can, $) {
  ...
  var model = controller.options.model, page = new $.Deferred()
    ;

  pager = model.findPage(params).then(function (results) {
    var collection_name = model.root_collection + '_collection', collection = results[collection_name] || []
      ;
  ...
  defaults: {
    is_related: false, model: null, extra_params: null, search_query: '', search_params: null, parent_instance: null, parent_type: null, object_display: null, parent_display: null, header_view: null, list_view: '/static/mustache/dashboard/object_list.mustache', list_objects: null, list_loader: null, tooltip_view: '/static/mustache/dashboard/object_tooltip.mustache'
  }
}, {
  ...
  init: function () {
    ...
    var that = this, params = $.extend({}, this.options.extra_params || {})
      ;
    ...
    var extra_params = this.options.extra_params || {}, search_params = this.options.search_params, that = this, page, pager
      ;
  ...
  }, '{list} change': 'update_count', '.view-more-paging click': function (el, ev) {
    var that = this, collection_name = that.options.model.root_collection + '_collection', is_next = el.data('next'), can_load = is_next ? that.options.pager.has_next() : that.options.pager.has_prev(), load = is_next ? that.options.pager.next : that.options.pager.prev;
    ...
    if (can_load) {
      ...
      this.fetch_list().then(this.proxy('draw_list'));
    }, '.search-filters button[type=reset] click': 'reset_search', '.btn-add modal:success': 'reset_search'
}
);
