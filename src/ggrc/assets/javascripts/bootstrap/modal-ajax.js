...
! function ($) {
  ...
  function preload_content() {
    var template =
      ['<div class="modal-header">', '  <a class="pull-right modal-dismiss" href="#" data-dismiss="modal">', '    <i class="grcicon-x-grey"></i>', '  </a>', '  <h2>Loading...</h2>', '</div>', '<div class="modal-body" style="padding-top:150px;"></div>', '<div class="modal-footer">', '</div>'
      ];
  ...
  var handlers = {
    ...
    'listnewform': function ($target, $trigger, option) {
      ...
      var list_target = $trigger.data('list-target'), selector_target = $trigger.data('selector-target')
        ;
    ...
    'deleteform': function ($target, $trigger, option) {
      var form_target = $trigger.data('form-target'), model = CMS.Models[$trigger.attr('data-object-singular')], instance, delete_counts = new can.Observe({loading: true, counts: ''})
        ;
      ...
      .ggrc_controllers_delete({
        $trigger: $trigger, skip_refresh: !$trigger.data('refresh'), new_object_form: false, button_view: GGRC.mustache_path + '/modals/delete_cancel_buttons.mustache', model: model, instance: instance, delete_counts: delete_counts, modal_title: 'Delete ' + $trigger.attr('data-object-singular'), content_view: GGRC.mustache_path + '/base_objects/confirm_delete.mustache'
      });
    ...
    'unmapform': function ($target, $trigger, option) {
      var form_target = $trigger.data('form-target'), object_params = $trigger.attr('data-object-params'), model = CMS.Models[$trigger.attr('data-object-singular')], instance;
      if ($trigger.attr('data-object-id') === 'page') {
        instance = GGRC.page_instance();
      } else {
        instance = model.findInCacheById($trigger.attr('data-object-id'));
      ...
      .ggrc_controllers_unmap({
        $trigger: $trigger, new_object_form: false, button_view: GGRC.mustache_path + '/modals/unmap_cancel_buttons.mustache', model: model, instance: instance, object_params: object_params, modal_title: $trigger.attr('data-modal-title') || ('Delete ' + $trigger.attr('data-object-singular')), content_view: $trigger.attr('data-content-view') || (GGRC.mustache_path + '/base_objects/confirm_unmap.mustache')
      });
    ...
    'form': function ($target, $trigger, option) {
      var form_target = $trigger.data('form-target'), object_params = $trigger.attr('data-object-params'), model = CMS.Models[$trigger.attr('data-object-singular')] || CMS.ModelHelpers[$trigger.attr('data-object-singular')], mapping = $trigger.data('mapping'), instance;
      ...
      .ggrc_controllers_modals({
        new_object_form: !$trigger.attr('data-object-id'), object_params: object_params, button_view: GGRC.Controllers.Modals.BUTTON_VIEW_SAVE_CANCEL_DELETE, model: model, current_user: GGRC.current_user, instance: instance, modal_title: object_params.modal_title || modal_title, content_view: content_view, mapping: mapping, $trigger: $trigger
      });
    ...
    $(function () {
      $('body').on(
        ...
        function (e) {
          ...
          var $this = $(this), toggle_type = $(this).data('toggle'), modal_id, target, $target, option, href, new_target, modal_type;
