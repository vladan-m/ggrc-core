  ...
  can.Observe('CMS.ModelHelpers.ApprovalWorkflow', {
    ...
    save: function () {
      ...
      return aws_dfd.then(function (aws) {
        ...
        allSaved = function () {
          var mapped;
          var savesDfd = function (tg) {
            return tg.attr('contact', that.contact)
              .save()
              .then(function (tg) {
                var tgtSaveDfd = function (tgt) {
                ...
                var tgtSavesDeferred = can.map(
                  tg.task_group_tasks.reify(),
                  tgtSaveDfd);
                return $.when.apply($, tgtSavesDeferred);
              });
          };
          mapped = can.map(can.makeArray(arguments), savesDfd);
        ...
        if (aws.length < 1) {
          ret = $.when(
            new CMS.Models.Workflow({
              ...
              notify_custom_message:
                ...
                that.original_object.constructor.model_singular +
                ' "' + that.original_object.title + '" before ' +
                moment(that.end_date).format('MM/DD/YYYY') + '. ' +
          ...
          instRefreshDfd = aws[0].instance.refresh();

          tgRefreshed = can.map(
            aws[0].instance.task_groups.reify(),
            function (tg) {
              return tg.refresh();
            });

          allSavedDfd = $.when.apply($, tgRefreshed).then(allSaved);
          ret = $.when(instRefreshDfd, allSavedDfd);
        }
