...
(function (root, $, can) {
    ...
    if (/^\/api\/\w+/.test(options.url)) {
      ...
      jqXHR.done(function (data, status, xhr) {
        ...
        switch (options.type.toUpperCase()) {
          case 'GET':
          case 'PUT':
            etags[originalOptions.url] = [xhr.getResponseHeader('ETag'),
              xhr.getResponseHeader('Last-Modified')];
            break;
          case 'POST':
            for (d in data) {
              if (data.hasOwnProperty(d) && data[d].selfLink) {
                etags[data[d].selfLink] = [xhr.getResponseHeader('ETag'),
                  xhr.getResponseHeader('Last-Modified')];
              }
            }
            break;
  ...
  can.ajax = $.ajax = function (options) {
    ...
    function setup(_new_ajax, _old_ajax) {
      ...
      if (_old_ajax) {
        _new_ajax.hasFailCallback = _old_ajax.hasFailCallback;
      }
  ...
  $doc.ajaxError(function (event, jqxhr, settings, exception) {
    if (!jqxhr.hasFailCallback || settings.flashOnFail ||
      (settings.flashOnFail === null && jqxhr.flashOnFail)) {
      ...
      if (settings.url.indexOf('import') === -1 || exception !== 'canceled') {
        $body.trigger('ajax:flash', {
          error: jqxhr.getResponseHeader('X-Flash-Error') ||
            statusmsgs[jqxhr.status] || exception.message || exception
        });
      }
    }
  });
