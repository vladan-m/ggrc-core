...
(function (can, $) {
  ...
  ' updateCount': function (el, ev, count, updateCount) {
    ...
    if (this.inner_nav_controller) {
      ...
      var $element = $('<section class=\'widget\'>'), control = new descriptor.controller(
      ...
      var $container = this.get_active_widget_containers().eq(0), $last_widget = $container.find('section.widget').last()
        ;
  ...
  init: function () {
    ...
    this._super();
      ...
      var that = this, that_super = this._super
        ;
  ...
  defaults: {
    internav_view: '/static/mustache/dashboard/internav_list.mustache', pin_view: '.pin-content', widget_list: null, spinners: {}, contexts: null, instance: null
  }
}, {
  init: function (options) {
    CMS.Models.DisplayPrefs.getSingleton().then(function (prefs) {
        ...
        can.view(this.options.internav_view, this.options, function (frag) {
          ...
          this.delayed_display = {
            fn: fn, timeout: setTimeout(fn, 50)
          };
        }.bind(this));
    ...
    var starttime = Date.now(), widget_list = this.options.widget_list.slice(0), that = this
        ;

    can.each(widget_elements, function (widget_element, index) {
        widget_list.splice(
          can.inArray(
            that.update_widget(widget_element, index), widget_list), 1);
      });
    ...
    can.each(widget_list, function (widget) {
      ...
      var $widget = $(widget_element), widget = this.widget_by_selector('#' + $widget.attr('id')), $header = $widget.find('.header h2'), icon = $header.find('i').attr('class'), menuItem = $header.text().trim(), match = menuItem ? menuItem.match(/\s*(\S.*?)\s*(?:\((?:(\d+)|\.*)(\/\d+)?\))?$/) : {}, title = match[1], count = match[2] || undefined, existing_index
      ;
    ...
    // If the metadata is unrendered, find it via options
      if (!title) {
      var widget_options = $widget.control('dashboard_widgets').options, widget_name = widget_options.widget_name;
    ...
    // Only create the observable once, this gets updated elsewhere
      ...
      widget = new can.Observe({
        selector: '#' + $widget.attr('id'), count: count, has_count: count !== null
      });
    }
      existing_index = this.options.widget_list.indexOf(widget);

      widget.attr({
      internav_icon: icon, internav_display: title, spinner: this.options.spinners['#' + $widget.attr('id')]
    });
